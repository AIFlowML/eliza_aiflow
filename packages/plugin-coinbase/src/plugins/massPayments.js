...
export const massPayoutProvider = {
...
async function executeMassPayout(runtime, networkId, receivingAddresses, transferAmount, assetId) {
    ...
    try {
        ...
        for (const address of receivingAddresses) {
            ...
            if (address) {
                try {
                    ...
                    elizaLogger.error("Error during transfer for address:", address, error);
                    transactions.push({
                        address,
                        amount: transferAmount,
                        status: "Failed",
        ...
        elizaLogger.error("Error initializing sending wallet or processing payouts:", error);
        throw error; // Re-throw the error to be caught in the handler
    }
}
// Action for sending mass payouts
export const sendMassPayoutAction = {
    ...
    validate: async (runtime, _message) => {
        ...
        return (!!(runtime.character.settings.secrets?.COINBASE_API_KEY ||
            process.env.COINBASE_API_KEY) &&
    ...
    handler: async (runtime, message, state, _options, callback) => {
        ...
        try {
            ...
            elizaLogger.log("Transfer details generated:", transferDetails.object);
...
- Asset ID is valid.`,
            ...
            const transactions = await executeMassPayout(runtime, network, receivingAddresses, transferAmount, assetId);
            ...
            const successDetails = successTransactions
                .map((tx) => `Address: ${tx.address}, Amount: ${tx.amount}, Transaction URL: ${tx.transactionUrl || "N/A"}`)
                .join("\n");
            const failedDetails = failedTransactions
                .map((tx) => `Address: ${tx.address}, Amount: ${tx.amount}, Error Code: ${tx.errorCode || "Unknown Error"}`)
                .join("\n");
            const charityTransactions = transactions.filter((tx) => tx.address === getCharityAddress(network));
...
Check the CSV file for full details.`,
            ...
            callback({ text: "Failed to complete payouts. Please try again." }, []);
        }
    },
    examples: [
        [
...
export const coinbaseMassPaymentsPlugin = {
...
//# sourceMappingURL=massPayments.js.map