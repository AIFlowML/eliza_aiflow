import { composeContext, elizaLogger, generateObjectV2, ModelClass, } from "@ai16z/eliza";
import { ChargeSchema, isChargeContent } from "../types";
import { chargeTemplate, getChargeTemplate } from "../templates";
import { getWalletDetails } from "../utils";
import { Coinbase } from "@coinbase/coinbase-sdk";
const url = "https://api.commerce.coinbase.com/charges";
export async function createCharge(apiKey, params) {
    try {
...
export async function getAllCharges(apiKey) {
    try {
        const response = await fetch(url, {
            method: "GET",
            headers: {
...
export async function getChargeDetails(apiKey, chargeId) {
    ...
    try {
        ...
        console.error(`Error fetching charge details for ID ${chargeId}:`, error);
        throw error;
    }
}
export const createCoinbaseChargeAction = {
    name: "CREATE_CHARGE",
    similes: [
        "MAKE_CHARGE",
        "INITIATE_CHARGE",
    ...
    handler: async (runtime, message, state, _options, callback) => {
        ...
        try {
            // Create a charge
            const chargeResponse = await createCharge(runtime.getSetting("COINBASE_COMMERCE_KEY"), {
                local_price: {
            ...
            elizaLogger.log("Coinbase Commerce charge created:", chargeResponse);
            callback({
            ...
            elizaLogger.error("Error creating Coinbase Commerce charge:", error);
            callback({
...
export const getAllChargesAction = {
    ...
    handler: async (runtime, message, state, _options, callback) => {
        try {
            ...
            const charges = await getAllCharges(runtime.getSetting("COINBASE_COMMERCE_KEY"));
...
export const getChargeDetailsAction = {
    ...
    handler: async (runtime, message, state, _options, callback) => {
        ...
        try {
            const chargeDetails = await getChargeDetails(runtime.getSetting("COINBASE_COMMERCE_KEY"), charge.id);
            ...
            elizaLogger.error(`Error fetching details for charge ID ${charge.id}:`, error);
            callback({
...
export const chargeProvider = {
    get: async (runtime, _message) => {
        ...
        const coinbaseAPIKey = runtime.getSetting("COINBASE_API_KEY") ??
            process.env.COINBASE_API_KEY;
        const coinbasePrivateKey = runtime.getSetting("COINBASE_PRIVATE_KEY") ??
...
export const coinbaseCommercePlugin = {
