...
export async function initializeWallet(runtime, networkId = Coinbase.networks.EthereumMainnet) {
    let wallet;
    const storedSeed = runtime.getSetting("COINBASE_GENERATED_WALLET_HEX_SEED") ??
        process.env.COINBASE_GENERATED_WALLET_HEX_SEED;
    const storedWalletId = runtime.getSetting("COINBASE_GENERATED_WALLET_ID") ??
    ...
    if (!storedSeed || !storedWalletId) {
        ...
        try {
            ...
            const walletIDSave = await updateCharacterSecrets(characterFilePath, "COINBASE_GENERATED_WALLET_ID", walletData.walletId);
            const seedSave = await updateCharacterSecrets(characterFilePath, "COINBASE_GENERATED_WALLET_HEX_SEED", walletData.seed);
            if (walletIDSave && seedSave) {
                ...
                elizaLogger.error(`Failed to update character secrets so adding gitignored ${seedFilePath} file please add it your env or character file and delete:`);
        ...
        elizaLogger.log("Imported existing wallet:", await wallet.getDefaultAddress());
    }
    return wallet;
}
/**
 ...
 */
export async function executeTradeAndCharityTransfer(runtime, network, amount, sourceAsset, targetAsset) {
...
export async function appendTradeToCsv(trade) {
    try {
...
export async function appendTransactionsToCsv(transactions) {
    try {
        const csvWriter = createArrayCsvWriter({
            path: csvFilePath,
            header: [
 ...
 */
export async function updateCharacterSecrets(characterfilePath, key, value) {
    try {
        const characterFilePath = path.resolve(process.cwd(), characterfilePath);
        ...
        fs.writeFileSync(characterFilePath, JSON.stringify(characterData, null, 2), "utf-8");
        console.log(`Updated ${key} in character.settings.secrets for ${characterFilePath}.`);
    }
    catch (error) {
        elizaLogger.error("Error updating character secrets:", error);
        return false;
...
export const getAssetType = (transaction) => {
    // Check for ETH
 ...
 */
export async function getWalletDetails(runtime, networkId = Coinbase.networks.EthereumMainnet) {
    try {
 ...
 */
export async function executeTransferAndCharityTransfer(wallet, amount, sourceAsset, targetAddress, network) {
    const charityAddress = getCharityAddress(network);
    const charityAmount = amount * 0.01;
 ...
 * @param {string} targetAddress - The target address to transfer to.
 */
export async function executeTransfer(wallet, amount, sourceAsset, targetAddress) {
    const assetIdLowercase = sourceAsset.toLowerCase();
    const transferDetails = {
        amount,
        assetId: assetIdLowercase,
 ...
 * @param {string} network - The network to use.
 */
export function getCharityAddress(network) {
    let charityAddress;
    if (network === "base") {
