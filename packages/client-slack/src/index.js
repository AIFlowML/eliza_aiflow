...
export class SlackClient extends EventEmitter {
        ...
        this.server.use((req, res, next) => {
            if (req.body) {
                ...
                try {
                    ...
                    elizaLogger.error("❌ [PARSE] Failed to parse request body:", error);
                }
            }
            next();
        });
    ...
    async verifyPermissions() {
        ...
        try {
            ...
            if (!testMessage.ok) {
                throw new Error(`Failed to send test message: ${testMessage.error}`);
            }
    ...
    async start() {
        try {
            ...
            this.server.post('/slack/events', async (req, res) => {
                try {
                    elizaLogger.debug("📥 [REQUEST] Incoming Slack event:", {
                        type: req.body?.type,
                    ...
                    if (req.body?.type === 'url_verification') {
                        elizaLogger.debug("🔑 [VERIFICATION] Challenge received:", req.body.challenge);
                    ...
                    if (req.body?.event) {
                        ...
                        elizaLogger.warn("⚠️ [EVENT] Received request without event data");
                    }
                    // Acknowledge receipt
                    res.status(200).send();
                }
            ...
            this.server.post('/slack/interactions', async (req, res) => {
                try {
                    elizaLogger.debug("🔄 [INTERACTION] Incoming interaction:", {
                        type: req.body?.type,
                    ...
                    elizaLogger.error("❌ [ERROR] Error processing interaction:", error);
                    res.status(500).json({ error: 'Internal server error' });
                }
            });
            // Start server
            ...
            this.server.listen(port, () => {
                elizaLogger.success(`🚀 [SERVER] Slack event server is running on port ${port}`);
                elizaLogger.success(`✅ [INIT] Slack client successfully started for character ${this.character.name}`);
                elizaLogger.success(`🤖 [READY] Bot user: @${auth.user} (${this.botUserId})`);
                elizaLogger.success(`📡 [EVENTS] Listening for events at: /slack/events`);
                elizaLogger.success(`💡 [INTERACTIONS] Listening for interactions at: /slack/interactions`);
                elizaLogger.success(`💡 [HELP] To interact with the bot:`);
                elizaLogger.success(`   1. Direct message: Find @${auth.user} in DMs`);
                elizaLogger.success(`   2. Channel: Mention @${auth.user} in any channel`);
            });
        }
        catch (error) {
