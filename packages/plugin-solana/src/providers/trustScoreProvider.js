...
export class TrustScoreManager {
    constructor(runtime, tokenProvider, trustScoreDb) {
        ...
        this.simulationSellingService = new SimulationSellingService(runtime, this.trustScoreDb);
    }
    //getRecommenederBalance
    async getRecommenederBalance(recommenderWallet) {
        try {
            const tokenAta = await getAssociatedTokenAddress(new PublicKey(recommenderWallet), this.baseMint);
     ...
     */
    async generateTrustScore(tokenAddress, recommenderId, recommenderWallet) {
        ...
        const decayFactor = Math.pow(this.DECAY_RATE, Math.min(inactiveDays, this.MAX_DECAY_DAYS));
        const decayedScore = recommenderMetrics.trustScore * decayFactor;
        const validationTrustScore = this.trustScoreDb.calculateValidationTrust(tokenAddress);
        return {
            tokenPerformance: {
    ...
    async updateRecommenderMetrics(recommenderId, tokenPerformance, recommenderWallet) {
        ...
        const overallTrustScore = this.calculateTrustScore(tokenPerformance, recommenderMetrics);
        const riskScore = this.calculateOverallRiskScore(tokenPerformance, recommenderMetrics);
        const consistencyScore = this.calculateConsistencyScore(tokenPerformance, recommenderMetrics);
        const balance = await this.getRecommenederBalance(recommenderWallet);
        const virtualConfidence = balance / 1000000; // TODO: create formula to calculate virtual confidence based on user balance
        const lastActive = recommenderMetrics.lastActiveDate;
        const now = new Date();
    ...
    async suspiciousVolume(tokenAddress) {
        const processedData = await this.tokenProvider.getProcessedTokenData();
     ...
     */
    async createTradePerformance(runtime, tokenAddress, recommenderId, data) {
        const recommender = await this.trustScoreDb.getOrCreateRecommenderWithTelegramId(recommenderId);
        const processedData = await this.tokenProvider.getProcessedTokenData();
        const wallet = new WalletProvider(this.connection, new PublicKey(Wallet));
        let tokensBalance = 0;
        ...
        this.simulationSellingService.processTokenPerformance(tokenAddress, recommenderId);
    ...
    async createTradeInBe(tokenAddress, recommenderId, data, retries = 3, delayMs = 2000) {
        for (let attempt = 1; attempt <= retries; attempt++) {
            try {
                await fetch(`${this.backend}/api/updaters/createTradePerformance`, {
                    method: "POST",
                    headers: {
     ...
     */
    async updateSellDetails(runtime, tokenAddress, recommenderId, sellTimeStamp, sellDetails, isSimulation) {
        const recommender = await this.trustScoreDb.getOrCreateRecommenderWithTelegramId(recommenderId);
        const processedData = await this.tokenProvider.getProcessedTokenData();
        const wallet = new WalletProvider(this.connection, new PublicKey(Wallet));
        const prices = await wallet.fetchPrices(runtime);
    ...
    async getRecommendations(startDate, endDate) {
        ...
        const result = Object.keys(groupedRecommendations).map((tokenAddress) => {
            ...
            tokenRecommendations.forEach((recommendation) => {
                const tokenPerformance = this.trustScoreDb.getTokenPerformance(recommendation.tokenAddress);
                const recommenderMetrics = this.trustScoreDb.getRecommenderMetrics(recommendation.recommenderId);
                const trustScore = this.calculateTrustScore(tokenPerformance, recommenderMetrics);
                const consistencyScore = this.calculateConsistencyScore(tokenPerformance, recommenderMetrics);
                const riskScore = this.calculateRiskScore(tokenPerformance);
...
export const trustScoreProvider = {
...
//# sourceMappingURL=trustScoreProvider.js.map