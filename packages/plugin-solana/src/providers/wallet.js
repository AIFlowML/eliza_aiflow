...
export class WalletProvider {
    constructor(connection, walletPublicKey) {
    ...
    async fetchPortfolioValue(runtime) {
        try {
            ...
            const portfolio = {
                ...
                items: items.sort((a, b) => new BigNumber(b.valueUsd)
    ...
    async fetchPortfolioValueCodex(runtime) {
        try {
            ...
            const totalUsd = items.reduce((sum, item) => sum.plus(new BigNumber(item.valueUsd)), new BigNumber(0));
            const totalSol = totalUsd.div(solPriceInUSD);
            const portfolio = {
    ...
    formatPortfolio(runtime, portfolio, prices) {
        ...
        const nonZeroItems = portfolio.items.filter((item) => new BigNumber(item.uiAmount).isGreaterThan(0));
...
const walletProvider = {
    get: async (runtime, _message, _state) => {
        try {
            ...
            if (!publicKey) {
                console.error("SOLANA_PUBLIC_KEY not configured, skipping wallet injection");
                return "";
            }
