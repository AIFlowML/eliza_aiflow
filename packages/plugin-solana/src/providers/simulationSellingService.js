...
export class SimulationSellingService {
    constructor(runtime, trustScoreDb) {
        ...
        this.MAX_DECAY_DAYS = 30;
        this.runningProcesses = new Set();
        this.trustScoreDb = trustScoreDb;
        this.connection = new Connection(runtime.getSetting("RPC_URL"));
        this.walletProvider = new WalletProvider(this.connection, new PublicKey(runtime.getSetting("SOLANA_PUBLIC_KEY") ??
            runtime.getSetting("WALLET_PUBLIC_KEY")));
        this.baseMint = new PublicKey(runtime.getSetting("BASE_MINT") ||
    ...
    async processMessage(message) {
        try {
            const { tokenAddress, amount, sell_recommender_id } = JSON.parse(message);
            console.log(`Received message for token ${tokenAddress} to sell ${amount}`);
            const decision = {
                tokenPerformance: await this.trustScoreDb.getTokenPerformance(tokenAddress),
                amountToSell: amount,
    ...
    async executeSellDecision(decision) {
        ...
        try {
            console.log(`Executing sell for token ${tokenPerformance.symbol}: ${amountToSell}`);
            ...
            const sellDetailsData = await this.updateSellDetails(tokenAddress, sell_recommender_id, sellTimeStamp, sellDetails, true, // isSimulation
            tokenProvider);
            ...
            console.error(`Error executing sell for token ${tokenAddress}:`, error);
        }
    }
    async startService() {
    ...
    processTokenPerformances(tokenPerformances) {
        ...
        tokenPerformances = tokenPerformances.filter((tp) => !runningProcesses.has(tp.tokenAddress));
        ...
        tokenPerformances.forEach(async (tokenPerformance) => {
            ...
            // if (shouldTrade) {
            const tokenRecommendations = this.trustScoreDb.getRecommendationsByToken(tokenPerformance.tokenAddress);
            ...
            const tokenAddress = tokenPerformance.tokenAddress;
            const process = await this.startProcessInTheSonarBackend(tokenAddress, balance, true, sell_recommender_id, tokenPerformance.initialMarketCap);
            if (process) {
    ...
    processTokenPerformance(tokenAddress, recommenderId) {
        try {
            const runningProcesses = this.runningProcesses;
            ...
            const tokenPerformance = this.trustScoreDb.getTokenPerformance(tokenAddress);
            // eslint-disable-next-line @typescript-eslint/no-unused-vars
            const tokenProvider = new TokenProvider(tokenPerformance.tokenAddress, this.walletProvider, this.runtime.cacheManager);
            const balance = tokenPerformance.balance;
            const sell_recommender_id = recommenderId;
            const process = this.startProcessInTheSonarBackend(tokenAddress, balance, true, sell_recommender_id, tokenPerformance.initialMarketCap);
            if (process) {
                this.runningProcesses.add(tokenAddress);
            }
        }
        catch (error) {
            console.error(`Error getting token performance for token ${tokenAddress}:`, error);
        }
    }
    async startProcessInTheSonarBackend(tokenAddress, balance, isSimulation, sell_recommender_id, initial_mc) {
        try {
            ...
            const response = await fetch(`${this.sonarBe}/ai16z-sol/startProcess`, {
                method: "POST",
                headers: {
            ...
            if (!response.ok) {
                console.error(`Failed to send message to process token ${tokenAddress}`);
                return;
            }
            ...
            console.error(`Error sending message to process token ${tokenAddress}:`, error);
            return null;
        }
    }
    stopProcessInTheSonarBackend(tokenAddress) {
        try {
            ...
            console.error(`Error stopping process for token ${tokenAddress}:`, error);
        }
    }
    async updateSellDetails(tokenAddress, recommenderId, sellTimeStamp, sellDetails, isSimulation, tokenProvider) {
        const recommender = await this.trustScoreDb.getOrCreateRecommenderWithTelegramId(recommenderId);
        ...
        const trade = await this.trustScoreDb.getLatestTradePerformance(tokenAddress, recommender.id, isSimulation);
        ...
        this.trustScoreDb.updateTradePerformanceOnSell(tokenAddress, recommender.id, buyTimeStamp, sellDetailsData, isSimulation);
        ...
        this.updateTradeInBe(tokenAddress, recommender.id, recommender.telegramId, sellDetailsData, tokenBalance);
        return sellDetailsData;
    }
    async isRapidDump(tokenAddress, tokenProvider) {
        const processedData = await tokenProvider.getProcessedTokenData();
    ...
    async updateTradeInBe(tokenAddress, recommenderId, username, data, balanceLeft, retries = 3, delayMs = 2000) {
        for (let attempt = 1; attempt <= retries; attempt++) {
            try {
                await fetch(`${this.backend}/api/updaters/updateTradePerformance`, {
                    method: "POST",
                    headers: {
                        "Content-Type": "application/json",
                ...
                console.error(`Attempt ${attempt} failed: Error creating trade in backend`, error);
                if (attempt < retries) {
...
//# sourceMappingURL=simulationSellingService.js.map