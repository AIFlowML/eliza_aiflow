...
export class TokenProvider {
    ...
    async fetchTokenCodex() {
        try {
            ...
            if (cachedData) {
                console.log(`Returning cached token data for ${this.tokenAddress}.`);
                return cachedData;
            }
            const query = `
          ...
          `;
            ...
            console.error("Error fetching token data from Codex:", error.message);
            return {};
        }
    }
    async fetchPrices() {
        try {
            ...
            for (const token of tokens) {
                const response = await this.fetchWithRetry(`${PROVIDER_CONFIG.BIRDEYE_API}/defi/price?address=${token}`, {
                    headers: {
                        "x-chain": "solana",
                    },
                });
    ...
    async fetchTokenSecurity() {
        ...
        const cachedData = this.getCachedData(cacheKey);
        if (cachedData) {
            console.log(`Returning cached token security data for ${this.tokenAddress}.`);
            return cachedData;
        }
    ...
    async fetchTokenTradeData() {
        ...
        const cachedData = this.getCachedData(cacheKey);
        if (cachedData) {
            console.log(`Returning cached token trade data for ${this.tokenAddress}.`);
            return cachedData;
        }
    ...
    async analyzeHolderDistribution(tradeData) {
        ...
        const averageChange = validChanges.reduce((acc, curr) => acc + curr, 0) /
            validChanges.length;
    ...
    async fetchHolderList() {
        ...
        try {
            ...
            const holders = Array.from(allHoldersMap.entries()).map(([address, balance]) => ({
                address,
    ...
    async getProcessedTokenData() {
        try {
            ...
            console.log(`Analyzing holder distribution for token: ${this.tokenAddress}`);
            const holderDistributionTrend = await this.analyzeHolderDistribution(tradeData);
            console.log(`Filtering high-value holders for token: ${this.tokenAddress}`);
            ...
            console.log(`Counting high-supply holders for token: ${this.tokenAddress}`);
            const highSupplyHoldersCount = await this.countHighSupplyHolders(security);
            console.log(`Determining DexScreener listing status for token: ${this.tokenAddress}`);
...
const tokenProvider = {
    get: async (runtime, _message, _state) => {
        try {
            const walletProvider = new WalletProvider(connection, new PublicKey(PROVIDER_CONFIG.MAIN_WALLET));
            const provider = new TokenProvider(tokenAddress, walletProvider, runtime.cacheManager);
