import { composeContext, generateObjectArray, generateTrueOrFalse, MemoryManager, booleanFooter, ModelClass, } from "@ai16z/eliza";
...
async function handler(runtime, message) {
    ...
    for (const rec of filteredRecommendations) {
        ...
        const walletProvider = new WalletProvider(new Connection(runtime.getSetting("RPC_URL") ||
            "https://api.mainnet-beta.solana.com"), new PublicKey(runtime.getSetting("SOLANA_PUBLIC_KEY") ??
            runtime.getSetting("WALLET_PUBLIC_KEY")));
        const tokenProvider = new TokenProvider(rec.contractAddress, walletProvider, runtime.cacheManager);
        // TODO: Check to make sure the contract address is valid, it's the right one, etc
        //
        if (!rec.contractAddress) {
            const tokenAddress = await tokenProvider.getTokenFromWallet(runtime, rec.ticker);
        ...
        const user = participants.find(async (actor) => {
            ...
            return (user.name.toLowerCase().trim() ===
        ...
        switch (rec.type) {
            case "buy":
                ...
                await trustScoreManager.createTradePerformance(runtime, rec.contractAddress, userId, {
