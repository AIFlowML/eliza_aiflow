...
import { settings, ModelClass, generateObject, composeContext, } from "@ai16z/eliza";
import { walletProvider } from "../providers/wallet.ts";
export function isCreateAndBuyContent(runtime, content) {
    console.log("Content for create & buy", content);
    return (typeof content.tokenMetadata === "object" &&
...
export const createAndBuyToken = async ({ deployer, mint, tokenMetadata, buyAmountSol, priorityFee, allowOffCurve, commitment = "finalized", sdk, connection, slippage, }) => {
    const createResults = await sdk.createAndBuy(deployer, mint, tokenMetadata, buyAmountSol, BigInt(slippage), priorityFee, commitment);
...
export const buyToken = async ({ sdk, buyer, mint, amount, priorityFee, allowOffCurve, slippage, connection, }) => {
...
export const sellToken = async ({ sdk, seller, mint, amount, priorityFee, allowOffCurve, slippage, connection, }) => {
...
export default {
    ...
    handler: async (runtime, message, state, _options, callback) => {
        ...
        try {
            ...
            const lamports = Math.floor(Number(buyAmountSol) * 1000000000);
