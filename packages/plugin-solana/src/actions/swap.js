...
import { ModelClass, composeContext, generateObject, settings, } from "@ai16z/eliza";
...
async function getTokensInWallet(runtime) {
    const walletProvider = new WalletProvider(new Connection("https://api.mainnet-beta.solana.com"), new PublicKey(runtime.getSetting("SOLANA_PUBLIC_KEY") ??
...
export const executeSwap = {
    ...
    handler: async (runtime, message, state, _options, callback) => {
        ...
        try {
            ...
            const walletPublicKey = new PublicKey(runtime.getSetting("SOLANA_PUBLIC_KEY") ??
                runtime.getSetting("WALLET_PUBLIC_KEY"));
            ...
            const expectedPublicKey = runtime.getSetting("SOLANA_PUBLIC_KEY") ??
                runtime.getSetting("WALLET_PUBLIC_KEY");
            ...
            if (type === "buy") {
                const tokenProvider = new TokenProvider(response.outputTokenCA, provider, runtime.cacheManager);
                ...
                const trustScoreDatabase = new TrustScoreManager(runtime, tokenProvider, trustScoreDb);
                // save the trade
                const tradeData = {
                    buy_amount: response.amount,
                    is_simulation: false,
                };
                await trustScoreDatabase.createTradePerformance(runtime, response.outputTokenCA, recommender.id, tradeData);
            }
            else if (type === "sell") {
                const tokenProvider = new TokenProvider(response.inputTokenCA, provider, runtime.cacheManager);
                const module = await import("better-sqlite3");
                const Database = module.default;
                const trustScoreDb = new TrustScoreDatabase(new Database(":memory:"));
                // add or get recommender
                ...
                await trustScoreDatabase.updateSellDetails(runtime, response.inputTokenCA, recommender.id, sellTimeStamp, sellDetails, false);
            }
