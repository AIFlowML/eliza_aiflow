...
async function invokeSwapDao(connection, authority, statePDA, walletPDA, instructionData) {
    ...
    const combinedData = new Uint8Array(discriminator.length + instructionData.length);
...
export const executeSwapForDAO = {
    ...
    handler: async (runtime, message) => {
        ...
        try {
            ...
            const authority = Keypair.fromSecretKey(Uint8Array.from(Buffer.from(runtime.getSetting("SOLANA_PRIVATE_KEY") ??
            ...
            // Derive PDAs
            const [statePDA] = await PublicKey.findProgramAddress([Buffer.from("state"), daoMint.toBuffer()], authority.publicKey);
            const [walletPDA] = await PublicKey.findProgramAddress([Buffer.from("wallet"), daoMint.toBuffer()], authority.publicKey);
            const quoteData = await getQuote(connection, inputToken, outputToken, amount);
            ...
            const txid = await invokeSwapDao(connection, authority, statePDA, walletPDA, instructionData);
...
//# sourceMappingURL=swapDao.js.map