import { ModelClass, ServiceType, composeContext, elizaLogger, getEmbeddingZeroVector, generateMessageResponse, stringToUuid, generateShouldRespond, } from "@ai16z/eliza";
import { NoSubscriberBehavior, StreamType, VoiceConnectionStatus, createAudioPlayer, createAudioResource, getVoiceConnection, joinVoiceChannel, entersState, } from "@discordjs/voice";
import { ChannelType, } from "discord.js";
import EventEmitter from "events";
import prism from "prism-media";
import { pipeline } from "stream";
import { discordShouldRespondTemplate, discordVoiceHandlerTemplate, } from "./templates.ts";
...
export class VoiceManager extends EventEmitter {
    ...
    async joinChannel(channel) {
        ...
        try {
            ...
            elizaLogger.log(`Voice connection established in state: ${connection.state.status}`);
            // Set up ongoing state change monitoring
            connection.on("stateChange", async (oldState, newState) => {
                elizaLogger.log(`Voice connection state changed from ${oldState.status} to ${newState.status}`);
                ...
                else if (!this.connections.has(channel.id) &&
    ...
    async monitorMember(member, channel) {
        ...
        opusDecoder.on("data", (pcmData) => {
            ...
            if (this.activeAudioPlayer) {
                ...
                const avgVolume = volumeBuffer.reduce((sum, v) => sum + v, 0) /
                    VOLUME_WINDOW_SIZE;
        ...
        this.client.emit("userStream", userId, name, userName, channel, opusDecoder);
    }
    ...
    async handleUserStream(userId, name, userName, channel, audioStream) {
        ...
        const debouncedProcessTranscription = debounce(async () => {
            await this.processTranscription(userId, channelId, channel, name, userName);
    ...
    async processTranscription(userId, channelId, channel, name, userName) {
        ...
        try {
            ...
            if (state.transcriptionText.length) {
                ...
                await this.handleUserMessage(finalText, userId, channelId, channel, name, userName);
            }
        }
        catch (error) {
            console.error(`Error transcribing audio for user ${userId}:`, error);
        }
    }
    async handleUserMessage(message, userId, channelId, channel, name, userName) {
        try {
            ...
            const shouldRespond = await this._shouldRespond(message, userId, channel, state);
            if (!shouldRespond) {
                return;
            }
            ...
            const callback = async (content) => {
                ...
                if (responseMemory.content.text?.trim()) {
                    ...
                    if (responseStream) {
                        await this.playAudioStream(userId, responseStream);
                    }
    ...
    async _shouldRespond(message, userId, channel, state) {
        ...
        if (lowerMessage.includes(botName) ||
            ...
            lowerMessage.includes(this.client.user?.tag.toLowerCase()) ||
    ...
    async scanGuild(guild) {
        ...
        try {
            ...
            if (!chosenChannel) {
                const channels = (await guild.channels.fetch()).filter((channel) => channel?.type == ChannelType.GuildVoice);
    ...
    async handleJoinChannelCommand(interaction) {
        try {
            ...
            const voiceChannel = interaction.guild.channels.cache.find((channel) => channel.id === channelId &&
