...
export class MessageManager {
    ...
    async handleMessage(message) {
        ...
        try {
            ...
            const audioAttachments = message.attachments.filter((attachment) => attachment.contentType?.startsWith("audio/"));
            ...
            if (!canSendResult.canSend) {
                return elizaLogger.warn(`Cannot send message to channel ${message.channel}`, canSendResult);
            }
            if (!shouldIgnore) {
            ...
            const agentUserState = await this.runtime.databaseAdapter.getParticipantUserState(roomId, this.runtime.agentId);
            ...
            else if ((!shouldRespond && hasInterest) ||
            ...
            if (shouldRespond) {
                ...
                const callback = async (content, files) => {
                    try {
                        ...
                        const messages = await sendMessageInChunks(message.channel, content.text, message.id, files);
                        const memories = [];
            ...
            if (message.channel.type === ChannelType.GuildVoice) {
                ...
                const speechService = this.runtime.getService(ServiceType.SPEECH_GENERATION);
                if (!speechService) {
    ...
    async processMessageMedia(message) {
        ...
        for (const url of urls) {
            if (this.runtime
                ...
                const browserService = this.runtime.getService(ServiceType.BROWSER);
                if (!browserService) {
    ...
    async _shouldIgnore(message) {
        ...
        messageContent = messageContent.replace(new RegExp(botMention, "gi"), this.runtime.character.name.toLowerCase());
        // Replace the bot's username with the character name
        const botUsername = this.client.user?.username.toLowerCase();
        messageContent = messageContent.replace(new RegExp(`\\b${botUsername}\\b`, "g"), this.runtime.character.name.toLowerCase());
        ...
        if (message.content.length < 4 &&
            ignoreResponseWords.some((word) => message.content.toLowerCase().includes(word))) {
            return true;
        }
        return false;
    }
    async _shouldRespond(message, state) {
        ...
        if (message.content
            .toLowerCase()
            .includes(this.client.user?.username.toLowerCase()) ||
            message.content
                .toLowerCase()
                .includes(this.client.user?.tag.toLowerCase()) ||
            (nickname &&
