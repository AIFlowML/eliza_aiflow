...
\`\`\`
`;
const getAttachmentIds = async (runtime, message, state) => {
...
const summarizeAction = {
    ...
    handler: async (runtime, message, state, options, callback) => {
        ...
        const attachments = state.recentMessagesData
            .filter((msg) => msg.content.attachments &&
            msg.content.attachments.length > 0)
            .flatMap((msg) => msg.content.attachments)
            // check by first 5 characters of uuid
            .filter((attachment) => attachmentIds
        ...
        const context = composeContext({
            ...
            template: trimTokens(summarizationTemplate, chunkSize + 500, "gpt-4o-mini" // TODO: make this dynamic and generic
            ),
        });
...
//# sourceMappingURL=chat_with_attachments.js.map