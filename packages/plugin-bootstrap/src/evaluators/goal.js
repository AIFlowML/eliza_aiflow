...
\`\`\``;
async function handler(runtime, message, state, options = { onlyInProgress: true }) {
    // get goals
        ...
        if (update) {
            ...
            if (update.objectives) {
                for (const objective of objectives) {
                    const updatedObjective = update.objectives.find((o) => o.description === objective.description);
                    if (updatedObjective) {
