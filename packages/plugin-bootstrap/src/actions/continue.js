...
export const continueAction = {
    ...
    validate: async (runtime, message) => {
        ...
        const agentMessages = recentMessagesData.filter((m) => m.userId === runtime.agentId);
        ...
        if (agentMessages) {
            ...
            if (lastMessages.length >= maxContinuesInARow) {
                const allContinues = lastMessages.every((m) => m.content.action === "CONTINUE");
                if (allContinues) {
                    return false;
                }
            }
    ...
    handler: async (runtime, message, state, options, callback) => {
        if (message.content.text.endsWith("?") ||
        ...
        async function _shouldContinue(state) {
        ...
        const context = composeContext({
            state,
            template: runtime.character.templates?.continueMessageHandlerTemplate ||
