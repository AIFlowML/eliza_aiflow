import { composeContext, generateText, getEmbeddingZeroVector, ModelClass, stringToUuid, parseBooleanFromText, } from "@ai16z/eliza";
...
export class TwitterPostClient {
    async start(postImmediately = false) {
        ...
        const generateNewTweetLoop = async () => {
            const lastPost = await this.runtime.cacheManager.get("twitter/" +
            ...
            const randomMinutes = Math.floor(Math.random() * (maxMinutes - minMinutes + 1)) +
                minMinutes;
        ...
        if (this.runtime.getSetting("POST_IMMEDIATELY") != null &&
    ...
    async generateNewTweet() {
        ...
        try {
            ...
            await this.runtime.ensureUserExists(this.runtime.agentId, this.client.profile.username, this.runtime.character.name, "twitter");
            ...
            try {
                ...
                const result = await this.client.requestQueue.add(async () => await this.client.twitterClient.sendTweet(content));
                ...
                await this.runtime.cacheManager.set(`twitter/${this.client.profile.username}/lastPost`, {
                    id: tweet.id,
                    timestamp: Date.now(),
                });
