...
export const isValidTweet = (tweet) => {
...
export async function buildConversationThread(tweet, client, maxReplies = 10) {
    ...
    async function processThread(currentTweet, depth = 0) {
        ...
        const memory = await client.runtime.messageManager.getMemoryById(stringToUuid(currentTweet.id + "-" + client.runtime.agentId));
        if (!memory) {
            const roomId = stringToUuid(currentTweet.conversationId + "-" + client.runtime.agentId);
            const userId = stringToUuid(currentTweet.userId);
            await client.runtime.ensureConnection(userId, roomId, currentTweet.username, currentTweet.name, "twitter");
            await client.runtime.messageManager.createMemory({
                ...
                roomId,
                userId: currentTweet.userId === client.profile.id
                    ? client.runtime.agentId
...
export async function sendTweet(client, content, roomId, twitterUsername, inReplyTo) {
    ...
    for (const chunk of tweetChunks) {
        const result = await client.requestQueue.add(async () => await client.twitterClient.sendTweet(chunk.trim(), previousTweetId));
