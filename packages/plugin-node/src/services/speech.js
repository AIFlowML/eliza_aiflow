...
async function getVoiceSettings(runtime) {
    ...
    return {
        ...
        elevenlabsModel: elevenlabsSettings?.model ||
            runtime.getSetting("ELEVENLABS_MODEL_ID") ||
            "eleven_monolingual_v1",
        elevenlabsStability: elevenlabsSettings?.stability ||
            runtime.getSetting("ELEVENLABS_VOICE_STABILITY") ||
            "0.5",
        // ... other ElevenLabs settings ...
        vitsVoice: voiceSettings?.model ||
            voiceSettings?.url ||
...
async function textToSpeech(runtime, text) {
    ...
    try {
        const response = await fetch(`https://api.elevenlabs.io/v1/text-to-speech/${elevenlabsVoiceId}/stream?optimize_streaming_latency=${runtime.getSetting("ELEVENLABS_OPTIMIZE_STREAMING_LATENCY")}&output_format=${runtime.getSetting("ELEVENLABS_OUTPUT_FORMAT")}`, {
            method: "POST",
            headers: {
        ...
        if (response) {
            ...
            if (runtime
                ...
                const withHeader = prependWavHeader(readable, 1024 * 1024 * 100, sampleRate, 1, 16);
                return withHeader;
            }
            else {
                return readable;
        ...
        if (error.message === "QUOTA_EXCEEDED") {
            ...
            if (audio instanceof Buffer) {
                ...
                const wavHeaderBuffer = getWavHeader(pcmBuffer.length * 2, sampleRate, 1, 16);
...
SpeechService.serviceType = ServiceType.SPEECH_GENERATION;
