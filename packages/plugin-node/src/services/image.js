...
import { ModelProviderName, ServiceType, } from "@ai16z/eliza";
...
export class ImageDescriptionService extends Service {
    ...
    async requestOpenAI(imageUrl, imageData, prompt, isGif) {
        for (let attempt = 0; attempt < 3; attempt++) {
            try {
                ...
                elizaLogger.error(`OpenAI request failed (attempt ${attempt + 1}):`, error);
                if (attempt === 2)
                    throw error;
            }
        }
    ...
    async extractFirstFrameFromGif(gifUrl) {
        ...
        const tempFilePath = path.join(os.tmpdir(), `gif_frame_${Date.now()}.png`);
...
ImageDescriptionService.serviceType = ServiceType.IMAGE_DESCRIPTION;
