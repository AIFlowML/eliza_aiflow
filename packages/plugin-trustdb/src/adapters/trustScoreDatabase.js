...
export class TrustScoreDatabase {
    constructor(db) {
        ...
        const tables = this.db
            .prepare("SELECT name FROM sqlite_master WHERE type='table' AND name IN ('recommenders', 'recommender_metrics', 'token_performance', 'token_recommendations', 'recommender_metrics_history');")
            .all();
        if (tables.length !== 5) {
  ...
  `);
    ...
    addRecommender(recommender) {
        ...
        try {
            ...
            const result = this.db
                .prepare(sql)
                .run(id, recommender.address, recommender.solanaPubkey || null, recommender.telegramId || null, recommender.discordId || null, recommender.twitterId || null, recommender.ip || null);
     ...
     */
    getOrCreateRecommender(recommender) {
        try {
            // Begin a transaction
            const transaction = this.db.transaction(() => {
                ...
                const metricsInitialized = this.initializeRecommenderMetrics(newRecommenderId);
     ...
     * @returns Recommender object with all details, or null if failed
     */
    async getOrCreateRecommenderWithDiscordId(discordId) {
        try {
            // Begin a transaction
            const transaction = this.db.transaction(() => {
                // Attempt to retrieve the recommender
            ...
            console.error("Error in getOrCreateRecommenderWithDiscordId:", error);
            return null;
        }
    }
    /**
     ...
     * @returns Recommender object with all details, or null if failed
     */
    async getOrCreateRecommenderWithTelegramId(telegramId) {
        try {
            // Begin a transaction
            const transaction = this.db.transaction(() => {
                // Attempt to retrieve the recommender
            ...
            console.error("Error in getOrCreateRecommenderWithTelegramId:", error);
            return null;
        }
    }
    /**
     ...
     */
    initializeRecommenderMetrics(recommenderId) {
        const sql = `
     ...
     */
    getRecommenderMetrics(recommenderId) {
        const sql = `SELECT * FROM recommender_metrics WHERE recommender_id = ?;`;
        const row = this.db.prepare(sql).get(recommenderId);
        if (!row)
            return null;
        return {
     ...
     * @param recommenderId Recommender's UUID
     */
    logRecommenderMetricsHistory(recommenderId) {
        ...
        try {
            this.db
                .prepare(sql)
                .run(history.historyId, history.recommenderId, history.trustScore, history.totalRecommendations, history.successfulRecs, history.avgTokenPerformance, history.riskScore, history.consistencyScore, history.recordedAt.toISOString());
            console.log(`Logged metrics history for recommender ID: ${recommenderId}`);
        }
        catch (error) {
     ...
     */
    updateRecommenderMetrics(metrics) {
        ...
        try {
            this.db
                .prepare(sql)
                .run(metrics.trustScore, metrics.totalRecommendations, metrics.successfulRecs, metrics.avgTokenPerformance, metrics.riskScore, metrics.consistencyScore, metrics.recommenderId);
            console.log(`Updated metrics for recommender ID: ${metrics.recommenderId}`);
        }
        catch (error) {
     ...
     */
    upsertTokenPerformance(performance) {
        const validationTrust = this.calculateValidationTrust(performance.tokenAddress);
        const sql = `
        ...
        `;
        try {
            this.db.prepare(sql).run(performance.tokenAddress, performance.priceChange24h, performance.volumeChange24h, performance.trade_24h_change, performance.liquidity, performance.liquidityChange24h, performance.holderChange24h, // Ensure column name matches schema
            performance.rugPull ? 1 : 0, performance.isScam ? 1 : 0, performance.marketCapChange24h, performance.sustainedGrowth ? 1 : 0, performance.rapidDump ? 1 : 0, performance.suspiciousVolume ? 1 : 0, performance.balance, performance.initialMarketCap, validationTrust);
    ...
    getTokenPerformance(tokenAddress) {
        ...
        const row = this.db.prepare(sql).get(tokenAddress);
        if (!row)
            return null;
        return {
     ...
     */
    calculateValidationTrust(tokenAddress) {
        const sql = `
     ...
     * @returns boolean indicating success
     */
    addTokenRecommendation(recommendation) {
        const sql = `
        ...
        try {
            this.db
                .prepare(sql)
                .run(recommendation.id || uuidv4(), recommendation.recommenderId, recommendation.tokenAddress, recommendation.timestamp || new Date(), recommendation.initialMarketCap || null, recommendation.initialLiquidity || null, recommendation.initialPrice || null);
            return true;
        }
        catch (error) {
     ...
     */
    getRecommendationsByRecommender(recommenderId) {
        const sql = `SELECT * FROM token_recommendations WHERE recommender_id = ? ORDER BY timestamp DESC;`;
        const rows = this.db.prepare(sql).all(recommenderId);
        return rows.map((row) => ({
            id: row.id,
            recommenderId: row.recommender_id,
            tokenAddress: row.token_address,
     ...
     * @returns Array of TokenRecommendation objects
     */
    getRecommendationsByToken(tokenAddress) {
     ...
     * @returns Array of TokenRecommendation objects
     */
    getRecommendationsByDateRange(startDate, endDate) {
        const sql = `
     ...
     */
    getRecommenderMetricsHistory(recommenderId) {
        const sql = `
     ...
     * @returns boolean indicating success.
     */
    addTradePerformance(trade, isSimulation) {
  ...
  `;
        try {
            this.db
                .prepare(sql)
                .run(trade.token_address, trade.recommender_id, trade.buy_price, trade.sell_price || null, trade.buy_timeStamp, trade.sell_timeStamp || null, trade.buy_amount, trade.sell_amount || null, trade.buy_sol, trade.received_sol || null, trade.buy_value_usd, trade.sell_value_usd || null, trade.profit_usd || null, trade.profit_percent || null, trade.buy_market_cap, trade.sell_market_cap || null, trade.market_cap_change || null, trade.buy_liquidity, trade.sell_liquidity || null, trade.liquidity_change || null, trade.last_updated || new Date().toISOString(), trade.rapidDump ? 1 : 0);
     ...
     * @returns boolean indicating success.
     */
    updateTradePerformanceOnSell(tokenAddress, recommenderId, buyTimeStamp, sellDetails, isSimulation) {
        const tableName = isSimulation ? "simulation_trade" : "trade";
        const sql = `
        UPDATE ${tableName}
        SET
    ...
    `;
        try {
            ...
            if (result.changes === 0) {
                console.warn(`No trade found to update in ${tableName} for token: ${tokenAddress}, recommender: ${recommenderId}, buyTimeStamp: ${buyTimeStamp}`);
                return false;
            }
     ...
     */
    getTradePerformance(tokenAddress, recommenderId, buyTimeStamp, isSimulation) {
        const tableName = isSimulation ? "simulation_trade" : "trade";
        const sql = `SELECT * FROM ${tableName} WHERE token_address = ? AND recommender_id = ? AND buy_timeStamp = ?;`;
        const row = this.db
            .prepare(sql)
            .get(tokenAddress, recommenderId, buyTimeStamp);
        if (!row)
            return null;
        return {
     ...
     * @returns TradePerformance object or null
     */
    getLatestTradePerformance(tokenAddress, recommenderId, isSimulation) {
        const tableName = isSimulation ? "simulation_trade" : "trade";
        const sql = `
        ...
        LIMIT 1;
    `;
        const row = this.db.prepare(sql).get(tokenAddress, recommenderId);
        if (!row)
            return null;
        return {
            token_address: row.token_address,
    ...
    `;
        try {
            this.db
                .prepare(sql)
                .run(transaction.tokenAddress, transaction.transactionHash, transaction.type, transaction.amount, transaction.price, transaction.isSimulation, transaction.timestamp);
            return true;
        }
        catch (error) {
...
//# sourceMappingURL=trustScoreDatabase.js.map