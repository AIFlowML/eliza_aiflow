...
const bridgeSendCFX = async (secretKey, rpcUrl, espaceTo, amount) => {
...
export const bridgeTransfer = {
    ...
    handler: async (runtime, message, state, options, callback) => {
        ...
        const secretKey = runtime.getSetting("CONFLUX_CORE_PRIVATE_KEY");
        const rpcUrl = runtime.getSetting("CONFLUX_CORE_SPACE_RPC_URL");
        let success = false;
        try {
            const hash = await bridgeSendCFX(secretKey, rpcUrl, content.object.to, content.object.amount.toString());
            success = true;
            if (!callback) {
                return success;
            }
