...
import { PumpSchema, isPumpContent, isPumpBuyContent, isPumpCreateContent, isPumpSellContent, } from "../types";
...
export const confiPump = {
    ...
    handler: async (runtime, message, state, options, callback) => {
        ...
        const account = privateKeyToAccount(runtime.getSetting("CONFLUX_ESPACE_PRIVATE_KEY"));
        ...
        try {
            ...
            switch (contentObject.action) {
                case "CREATE_TOKEN":
                    ...
                    console.log("creating: ", contentObject.params.name, contentObject.params.symbol, contentObject.params.description);
                    data = encodeFunctionData({
                        abi: MEMEABI,
                ...
                case "SELL_TOKEN":
                    ...
                    const tokenAddress = getAddress(contentObject.params.tokenAddress);
                    console.log("selling: ", tokenAddress, account.address, contentObject.params.value);
                    const amountUnits = parseUnits(contentObject.params.value.toString(), 18);
                    await ensureAllowance(walletClient, rpcUrl, account, tokenAddress, runtime.getSetting("CONFLUX_MEME_CONTRACT_ADDRESS"), amountUnits);
                    data = encodeFunctionData({
                        abi: MEMEABI,
                        functionName: "sell",
            ...
            const memeContractAddress = runtime.getSetting("CONFLUX_MEME_CONTRACT_ADDRESS");
...
//# sourceMappingURL=confiPump.js.map